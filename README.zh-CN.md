...
这是一个给 `Codex App` 用的技能（skill），用于在 macOS 和 Ubuntu 上配置 Visual Studio Code 的 LaTeX Workshop 深色 PDF 显示、右侧 PDF 标签页、`.tex` 自动换行，以及可选的 LaTeX 波浪线隐藏。
...
- 排查白色 PDF 渲染、旧 PDF 路径缓存，以及 `vscode-pdf` 扩展冲突
