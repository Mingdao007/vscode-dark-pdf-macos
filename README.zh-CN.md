...
- 反向 SyncTeX 还要求 PDF 旁边存在对应的 `.synctex.gz`。如果你走的是 LaTeX Workshop 默认的 `Build LaTeX project` 编译链，通常本来就会带上 SyncTeX；如果你改成了自定义 `tools`、自定义 `recipes`，或者外部脚本编译，就要确保以后每次编译都保留 `-synctex=1`，不能把它当成一次性补救。
