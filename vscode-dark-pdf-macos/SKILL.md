...
description: Configure macOS Visual Studio Code for dark PDF viewing in LaTeX Workshop, right-side PDF tabs, source wrap, and optional LaTeX squiggle suppression.
---

# VS Code Dark PDF on macOS

## Overview

Use LaTeX Workshop's internal PDF viewer first. Keep the configuration minimal and deterministic so the dark-viewer path stays predictable.
...
```bash
python3 scripts/apply_vscode_dark_pdf.py \
...
5. Relaunch VS Code on macOS with GPU disabled and open the `.tex` file plus a fresh-path PDF copy:

```bash
open -na "Visual Studio Code" --args --disable-gpu "/path/to/file.tex" "/path/to/fresh-copy.pdf"
```
...
- PDF opens but remains bright: make sure the test used a new PDF path and a GPU-disabled relaunch.
